    ...
    const valueProposition = [
        {
            ...
            description: "Flexible and fair pricing helps your event budget go further. There are no contracts or tiers. Just the same great service and support for everyone. Save 73% on your ticketing vs our lead competitor*."
        },
        {
            icon:  receive,
            title: "Easy-to-use",
            description: "You’re busy planning your event, so let us make the ticketing easy. Our feature-rich platform is simple, yet powerful, making anything possible. And if you need our help, we’re on hand 24/7."
        },
        {
            icon: device,
            title: "More than ticketing",
            description: "What if the next ticket you sold could have a positive impact on the planet? With Ticket Tailor it can. We’re not climate-experts, or as legendary as Attenborough, but you would be choosing."
        },
    ]
  return (
      <div className="relative">
          <div className={"px-5 md:px-10 lg:px-20 xl:px-[120px] bg-[#F8F8F8]"}>
              ...
              <div className={"grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 justify-between gap-x-10 gap-y-5 my-10"}>
          ...
          <div id={"about"} className={"my-20 px-5 md:px-10 lg:px-20 xl:px-[120px]"}>
              <div className={"flex justify-center text-center text-5xl text-[#171717] mb-14"}>
              ...
              <div className={"grid grid-cols-1 md:grid-cols-3 gap-6"}>
                  {
                      valueProposition.map(item => (<ValuePropositionCard key={item.title} item={item}/>))
                  }
              </div>
          </div>
          <div className={"bg-[#6A5BC1] h-[420px] mx-4 rounded-[32px] text-white flex justify-center items-center"}>
              <div>
                  <div className={"text-2xl md:text-[48px] font-medium mb-6 text-center"}>Sign up and get started</div>
                  <div className={"text-center"}>Our ticketing platform is made for you - and loved by everyone.</div>
                  <div className={"mt-[56px] flex justify-center relative text-white"}>
                      <Button text={"Create Event"} style={"bg-white py-3 px-5 bg-opacity-100 text-black rounded-2xl"}/>
                      <Image className={"absolute right-5 top-5 text-white fill-inherit h-20 md:h-32"} src={pointerImage} alt={""} />
                  </div>
              </div>
          </div>
